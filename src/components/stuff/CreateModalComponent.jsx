...
const CreateModalComponent = (props) => {
  ...
  const [input, setInput] = useState({ Outlet: auth.Outlet.id, Role: "stuff" });
  ...
  const handleClose = () => {
    setShow(false);
    setInput({ Outlet: auth.Outlet.id, Role: "stuff" });
    setInputError("");
  };
  ...
  const onClickHandler = () => {
    axios
      .post("http://127.0.0.1:8000/api/stuff/create/", input)
      .then((response) => {
